...
//New offer
	...
	if (!empty($DivCodeOrderOF))
	{
		$Query="SELECT orders.OrderID FROM cyrusproject.demand_details
		inner join orders on demand_details.OrderID=orders.OrderID
		Where demand_details.Status=1 and DivisionCode=$DivCodeOrderOF Group by demand_details.OrderID order by orders.OrderID";
		$result=mysqli_query($con,$Query);
		echo "<option value=''>Order ID</option>";
		if (mysqli_num_rows($result)>0)
		{
...
//Division Details
	...
	{   
		$query="SELECT * FROM cyrusproject.orders
		inner join pgtype on orders.PGTypeID=pgtype.PGTypeID
		inner join orderstatus on orders.OrderStatusID=orderstatus.StatusID WHERE DivisionCode=$OrderDetails";
		$result=mysqli_query($con,$query);
		if (mysqli_num_rows($result)>0)
		{
			...
			while ($row=mysqli_fetch_assoc($result))
			{
				...
				print '<td><a href="material.php?OrderID='.base64_encode($row["OrderID"]).'" target="_blank">'.$row["OrderID"].'</a></td>';
				print '<td>'.$row["Description"]."</td>";
				print '<td>'.$row["OrderNo"]."</td>";
				...
				print '<td>'.$row["PGType"]."</td>";
				print '<td><span class="d-none">'.$row["PGDate"].'</span>'.date('d-M-Y',strtotime($row["PGDate"]))."</td>";
				print '<td><span class="d-none">'.$row["PGValidDate"].'</span>'.date('d-M-Y',strtotime($row["PGValidDate"]))."</td>";
				print '<td>'.$row["OrderingAuth"]."</td>";
				print '<td>'.$row["BillingAuth"]."</td>";

				if($row["AMCIncluded"]==1){
	...
	$GetSiteAddress=!empty($_POST['GetSiteAddress'])?$_POST['GetSiteAddress']:'';
	if (!empty($GetSiteAddress))
	{
		$Query="SELECT Address FROM site WHERE SiteCode=$GetSiteAddress";
		$result=mysqli_query($con,$Query);
		if (mysqli_num_rows($result)>0)
		{
			$arr=mysqli_fetch_assoc($result);
	...
	$GetSiteDistrict=!empty($_POST['GetSiteDistrict'])?$_POST['GetSiteDistrict']:'';
	if (!empty($GetSiteDistrict))
	{
		$Query="SELECT DistrictName FROM site inner join district on site.DistrictCode=district.DistrictCode WHERE SiteCode=$GetSiteDistrict";
		$result=mysqli_query($con,$Query);
		if (mysqli_num_rows($result)>0)
		{
			$arr=mysqli_fetch_assoc($result);
			echo $arr['DistrictName'];

		}

	}

	$GetSiteConsignee=!empty($_POST['GetSiteConsignee'])?$_POST['GetSiteConsignee']:'';
	if (!empty($GetSiteConsignee))
	{
		$Query="SELECT Consignee FROM site WHERE SiteCode=$GetSiteConsignee";
		$result=mysqli_query($con,$Query);
		if (mysqli_num_rows($result)>0)
		{
			$arr=mysqli_fetch_assoc($result);
