...
$query="SELECT StartDate, MaterialName, sitework.SiteCode, Work, Unit, sitework.Qty, sitework.OrderID, sitework.WorkTypeID  FROM cyrusproject.sitework
...
$VQty=0;
$LeftQty=$WQty;
$query="SELECT sum(Qty) FROM cyrusproject.work_verification WHERE WorkID=$WorkID and AdditionalEmployee=0 and (VerificationRemark not like '%Rejected%' or VerificationRemark is null)";
$result=mysqli_query($con,$query);
if (mysqli_num_rows($result)>0)
{
	$row=mysqli_fetch_assoc($result);
...
if (isset($_POST['submit'])) {
	...
	$query="SELECT * FROM work_verification WHERE WorkID=$WorkID and WorkDate='$EntryDate' and Closed=0 and VerificationRemark not like '%Rejected%'";
	$result=mysqli_query($con,$query);
	if (mysqli_num_rows($result)>0)
	{
	...
	if (empty($errors)==true) {
		...
		if ($_POST['Labour']==1) {
			$LabourDetails=$_POST['LabourDetails'];
			$LabourQty=$_POST['LabourQty'];

			$sql = "INSERT INTO work_verification (EmployeeCode, SiteCode, OrderID, WorkID, WorkTypeID, Qty, WorkDate, LabourWork, Labours)
			VALUES ($EmployeeCode, $SiteCode, $OrderID, $WorkID, $WorkTypeID, $WorkQty, '$EntryDate', '$LabourDetails', $LabourQty)";

		}else{

			$sql = "INSERT INTO work_verification (EmployeeCode, SiteCode, OrderID, WorkID, WorkTypeID, Qty, WorkDate)
			VALUES ($EmployeeCode, $SiteCode, $OrderID, $WorkID, $WorkTypeID, $WorkQty, '$EntryDate')";
				...
				if (mysqli_num_rows($result)>0)
				{
					while ($row=mysqli_fetch_assoc($result))
					{
						...
						if ($_POST['Labour']==1) {
							$LabourDetails=$_POST['LabourDetails'];
							$LabourQty=$_POST['LabourQty'];
							$sql4 = "INSERT INTO work_verification (EmployeeCode, SiteCode, OrderID, WorkID, WorkTypeID, Qty, WorkDate, AdditionalEmployee, LabourWork, Labours)
							VALUES ($EmployeeCodeA, $SiteCode, $OrderID, $WorkID, $WorkTypeID, $WorkQty, '$EntryDate', 1, '$LabourDetails', $LabourQty)";
						}else{

							$sql4 = "INSERT INTO work_verification (EmployeeCode, SiteCode, OrderID, WorkID, WorkTypeID, Qty, WorkDate, AdditionalEmployee)
							VALUES ($EmployeeCodeA, $SiteCode, $OrderID, $WorkID, $WorkTypeID, $WorkQty, '$EntryDate', 1)";
						}
									...
									<div class="row">
										...
										<div class="col-lg-4 MC">
											...
											<div class="col-lg-2">
												...
												<h4 align="center" style="margin-top:10px;">
													<label class="form-check-label" for="flexCheckDefault">Labour/Third Party</label>
													<input class="form-check-input" type="checkbox" value="1" id="Labour" name="Labour">
												</h4>
												<center>
													<div class="col-lg-3">
														<label>Enter Name</label>
														<textarea class="form-control rounded-corner" type="text" maxlength="280" id="LabourDetails" name="LabourDetails" disabled></textarea>
													</div>
													<div class="col-lg-3">
														<label>No. of Labours</label>
														<input type="number" name="LabourQty" class="form-control rounded-corner" id="LabourQty" disabled>
													</div>
												</center>
												<h4 align="center" style="margin-top:10px;">
												...
												<center>
													...
													<div class="col-lg-3">
														...
														<div class="col-lg-3" style="margin-top:20px;">
															...
															<input type="date" min="<?php echo $StartDate ?>" max="<?php echo $Date ?>" class="form-control rounded-corner" name="EntryDate" required>
														</div>
