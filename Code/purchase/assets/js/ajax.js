...
$(document).on('change', '#DivisionCode', function(){
    ...
    if(DivisionCode){
        ...
        $.ajax({
          type:'POST',
          url:'select.php',
          data:{'OrderDetails':DivisionCode},
          success:function(result){

            $('.displayDetails').DataTable().clear();
            $('.displayDetails').DataTable().destroy();
            $('#OrderDetail').html(result);
            $('table.displayDetails').DataTable(); 

        }
    });

    }else{
        $('#SiteCode').html('<option value="">Division</option>');
    }
});
